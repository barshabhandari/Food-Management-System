...
def initiate_payment(
    ...
    existing_payment = db.query(Payment).filter(
        Payment.cart_id == payment_data.cart_id,
...
def payment_success(
    ...
    amt: str,
    refId: str,  # eSewa reference or mobile number
    db: Session = Depends(get_db)
):
    # Find payment by transaction_id
    payment = db.query(Payment).filter(Payment.transaction_id == oid).first()
    if not payment:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Invalid transaction ID")

    # Check if amount matches
    if float(amt) != payment.amount:
        payment.status = PaymentStatus.FAILED
        db.commit()
        return {"message": "Payment failed due to amount mismatch", "transaction_id": oid}

    # All good, set to completed
